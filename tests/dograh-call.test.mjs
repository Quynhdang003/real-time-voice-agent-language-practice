...
function harness(t, save) {
  ...
  let starts = 0, ends = 0, released = 0;
  ...
  const call = createVoiceCall({ widget, durationMinutes: 5,
    ...
    release: () => released++, now: () => 1000 });
  t.after(() => call.dispose());
  return { call, callbacks, events, states, starts: () => starts, ends: () => ends, released: () => released };
}
test("voice Start is synchronous and repeated clicks only start once", async (t) => {
  ...
  assert.equal(h.states.at(-1).status, "completed");
  assert.ok(h.released() > 0);
});
test("microphone rejection ends in a failed state", async (t) => {
  const h = harness(t);
  h.call.start();
...
test("failed persistence prevents later events overtaking it", async (t) => {
  const events = [];
